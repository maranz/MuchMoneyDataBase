DROP PROCEDURE IF EXISTS MZ_spMONEYl_all;

DELIMITER //
CREATE PROCEDURE MZ_spMONEYl_all (		
		 in YEAR int		
	    ,in APPID varchar(36)
)
BEGIN	 
	...
	insert into tmpMONEY
		SELECT YEAR, 13, 'TOTALE', U.USERID, U.USER, U.AVATAR, sum( M.MONEY )  MONEYIN, 0 MONEYOUT
		FROM MZ_MONEY M
		INNER JOIN MZ_USERS U on	
				U.CDATE IS null		
			AND U.USERID = M.USERID
	...
	insert into tmpMONEY
		SELECT Y.YEAR, Y.M, Y.MONTH, U.USERID, U.USER, U.AVATAR, SUM( M.MONEY ) MONEYIN, 0 MONEYOUT
		FROM MZ_MONEY M
		INNER JOIN MZ_USERS U on	
				U.CDATE IS null		
			AND U.USERID = M.USERID
